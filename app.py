...
def extract_tables(filepath):
    ...
    try:
        ...
        # Merge tables if they are split across pages
        merged_tables = []
        for table in tables:
            if merged_tables and merged_tables[-1].shape[1] == table.df.shape[1]:
                merged_tables[-1] = pd.concat([merged_tables[-1], table.df], ignore_index=True)
            else:
                merged_tables.append(table.df)
        
        logger.info(f"Extracted {len(merged_tables)} tables from the PDF.")
